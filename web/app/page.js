...
function ActionCard({ icon, title, desc }) {
  return (
    <div className="flex flex-col items-start bg-white border border-[#F2EDE4] rounded-lg p-4 gap-2 shadow-[0_2px_10px_rgba(0,0,0,0.02)] hover:bg-[#F9F7F2] transition-colors h-full">
...
function ActionLink({ icon, label }) {
  return (
    <button className="flex items-center gap-2 px-3 py-2.5 rounded-lg bg-white hover:bg-[#F9F7F2] text-sm font-semibold border border-[#F2EDE4] shadow-[0_2px_8px_rgba(0,0,0,0.01)] text-[#4A4E69] transition-all w-full text-left">
      {icon} {label}
    </button>
  );
}
